...
{
    public sealed partial class MoveToMenuItem : UserControl
    {
        public MoveToMenuItem()
        {
        ...
        public event EventHandler<MailMessageMovedEventArgs>? MailMessageMoved;


        public static readonly DependencyProperty MailServiceProperty =
            DependencyProperty.Register(nameof(MailService), typeof(IMailService), typeof(MoveToMenuItem), new PropertyMetadata(null, UpdateControlMenus));

        public static readonly DependencyProperty AllFoldersProperty =
            DependencyProperty.Register(nameof(AllFolders), typeof(TreeNode<MailFolder>), typeof(MoveToMenuItem), new PropertyMetadata(null, UpdateControlMenus));

        public static readonly DependencyProperty MessageProperty =
            DependencyProperty.Register(nameof(Message), typeof(MailMessage), typeof(MoveToMenuItem), new PropertyMetadata(null, UpdateControlMenus));
        ...
        private MenuFlyoutItem CreateMenuFlyoutSubItemOfMoveToCommand(MailFolder folderNode)
        {
            MenuFlyoutItem item = new MenuFlyoutItem()
            {
                ...
                Command = new AsyncRelayCommand<MailMessage>(async message =>
                {
                    ...
                    try
                    {
                        string originFolderId = message.ContainingFolderId;
                        await MailService.MoveMessageAsync(message, folderNode, default);

                        string newFolderId = message.ContainingFolderId;

                        MailMessageMoved?.Invoke(this, new MailMessageMovedEventArgs(message, originFolderId, newFolderId));
                    }
                    catch
                    {
        ...
        {
            if (d is not MoveToMenuItem menuItem)
            {
                return;
            }
